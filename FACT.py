...
# Configuración de la página para emular un dashboard compacto
    ...
    df = cargar_tabla_excel("Rep RIASA.xlsm", "Medidores Grafica", "Tabla6")
    ...
    df["Porcentaje_Ind"] = (df["Facturado"] / df["Objetivos"] * 100).fillna(0).round(1)

    # Totales globales para el Arco (Gauge)
    total_obj = df["Objetivos"].sum()
    total_real = df["Facturado"].sum()
    ...
    <div style="background-color: #1a1b23; border: 1px solid #2d2f39; border-radius: 12px; padding: 16px 20px; margin-bottom: 20px; box-shadow: 0 4px 6px rgba(0,0,0,0.15); width: 100%;">
        <p style="color: #808495; font-size: 26px; font-weight: bold; letter-spacing: 0.5px; margin: 0; text-transform: uppercase;">Facturación Total</p>
    ...
    with tarjeta_completa:
        ...
        with tarjeta_completa:
            for index, row in df.iterrows():
                ...
                col_val.markdown(f"<p style='text-align: right; margin:0;'>{row['Facturado']:,.0f} / {row['Objetivos']:,.0f}</p>", unsafe_allow_html=True)
    ...
    with tarjeta_completa:
        ...
        df_tabla = df[["MERCADO", "Objetivos", "Facturado"]].copy()
        df_tabla["Diferencia"] = df_tabla["Facturado"] - df_tabla["Objetivos"]

        # 2. Calcular e insertar la fila de totales
        fila_total = pd.DataFrame([{
            "MERCADO": "Total",
            "Objetivos": total_obj,
            "Facturado": total_real,
